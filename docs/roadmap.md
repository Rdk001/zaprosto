...
**Этап 06 завершён и принят после финального технического и security-аудита 06.7.**
Подтверждены одноразовые client/admin links, single-leader polling и transactional
offset, producer-матрица, outbox delivery/retry/rate gate, worker lifecycle,
операторские переходы, retention и защищённая наблюдаемость. Полные проверки:
859 unit, 1397 PostgreSQL integration и 135 Chromium E2E; миграции deploy/status
проверены на fresh isolated DB, реальные Telegram credentials/network не использовались.
[ADR-0014](decisions/0014-telegram-notifications.md) имеет статус Accepted.

**06.5C реализована.** Один уже захваченный outbox job проходит актуальный preflight,
не более одного вызова Telegram `sendMessage` и fenced `finish`. HTTP выполняется вне
PostgreSQL-транзакции; доставка имеет семантику at-least-once. Production dispatcher,
...
Финальная граница 06.7 закрыта. Весь MVP пока не объявляется готовым: следующим и
единственным оставшимся этапом является 07 — подготовка MVP к демонстрации.
...
**07.1 выполнена.** Критические клиентские и административные сценарии приняты в
Chromium на 360×800, 390×844, 412×915 и 1440×900: проверены responsive layout,
клавиатурный фокус, loading/empty/validation/conflict/unauthorized/unknown-outcome
состояния и fake/local Telegram без реальной сети. Добавлен
