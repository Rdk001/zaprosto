...
Реализация 06.6C использует один PostgreSQL timestamp на запуск и точные границы:
адресный rejection удаляется при возрасте `>= 24 часа`; остальные условия
«старше 30/90 дней» используют строгое `>`, поэтому строка ровно на границе
сохраняется до следующего запуска. Для token terminal time — первое из
`usedAt/revokedAt/expiresAt`; для активной клиентской связи — `endsAt`
прошедшей `SCHEDULED` записи либо `AppointmentStatusHistory.changedAt` текущего
terminal-статуса.
